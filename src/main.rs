...
fn gelu_scalar<F: Float>(x: Line<F>) -> Line<F> {
    ...
    let sqrt2 = F::new(comptime!(2.0f32.sqrt())); // TODO Cannot init to f64
...
pub fn launch<R: Runtime>(device: &R::Device) {
    ...
    let input = &[-1., 0., 1., 5.];
    let vectorization = 4;
    let output_handle = client.empty(input.len() * core::mem::size_of::<f64>());
    let input_handle = client.create(f64::as_bytes(input));

    unsafe {
        gelu_array::launch_unchecked::<f64, R>(
            &client,
            CubeCount::Static(1, 1, 1),
            CubeDim::new(input.len() as u32 / vectorization, 1, 1),
            ArrayArg::from_raw_parts::<f64>(&input_handle, input.len(), vectorization as u8),
            ArrayArg::from_raw_parts::<f64>(&output_handle, input.len(), vectorization as u8),
        )
    };

    let bytes = client.read_one(output_handle.binding());
    let output = f64::from_bytes(&bytes);
